...
#define SLEEP1_TIMEOUT_MS                                                      \
  300000 // 5 minutes to sleep1 from active (was 5 seconds)
#define SLEEP2_TIMEOUT_MS                                                      \
  600000 // 10 minutes to sleep2 from sleep1 (was 15 seconds)
#define SLEEP3_TIMEOUT_MS                                                      \
  900000 // 15 minutes to sleep3 from sleep2 (was 30 seconds)
#define ACTIVE_CONN_INTERVAL CONFIG_ZMK_SPLIT_BLE_PREF_INT
// Less aggressive intervals for better left side stability
#define SLEEP1_CONN_INTERVAL (CONFIG_ZMK_SPLIT_BLE_PREF_INT + 6)  // Was *2
#define SLEEP2_CONN_INTERVAL (CONFIG_ZMK_SPLIT_BLE_PREF_INT + 12) // Was *4
#define SLEEP3_CONN_INTERVAL (CONFIG_ZMK_SPLIT_BLE_PREF_INT + 18) // Was *8
#define CONN_LATENCY CONFIG_ZMK_SPLIT_BLE_PREF_LATENCY
// Keep latency low for all sleep modes
#define SLEEP1_CONN_LATENCY CONFIG_ZMK_SPLIT_BLE_PREF_LATENCY
#define SLEEP2_CONN_LATENCY CONFIG_ZMK_SPLIT_BLE_PREF_LATENCY
#define SLEEP3_CONN_LATENCY CONFIG_ZMK_SPLIT_BLE_PREF_LATENCY
#define SUPERVISION_TIMEOUT                                                    \
  600 // Increased from CONFIG_ZMK_SPLIT_BLE_PREF_TIMEOUT
...
static void power_mode_transition(struct k_work *work) {
  ...
  if (err == 0) {
    ...
    LOG_WRN("Failed to update connection parameters for %s mode: %d", mode_name,
            err);
  }
}
...
static void power_mgmt_bt_conn_disconnected_cb(struct bt_conn *conn,
                                               uint8_t reason) {
  if (conn != split_conn) {
    return;
  }
